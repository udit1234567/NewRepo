...
namespace AIA.Data.Life.API.ControllerLogic.UWRules
{
            ...
            try
            {
                ...
                using (AVOAIALifeEntities Context = new AVOAIALifeEntities())
                {
                    ...
                    if (IsDeviationCase)
                    {
                        ...
                        return "Your proposal has been forwarded to underwriter for further processing.";
                    }
                    else
                    {
                        return string.Empty;
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    switch (Parameter.ParameterName)
                                    {
                                        ...
                                        break;
                                        case "OtherDocUploaded":
                                        var Count = Context.tblPolicyDocuments.Where(a => a.PolicyID == objPolicy.PolicyID && a.MemberType == Member.AssuredName && a.ItemType == "PolicyDocuments" && a.FileName != "Age Proof").Count();
                                        if (Count > 0)
                                        {
                                            objItem.ParameterValue = "true";
                                        }
