...
namespace AIA.Life.Data.API.ControllerLogic.PremiumCalculation
{
    #region XML classes
    ...
    public class Premium : MapObject
    {
        ...
        public LifeQuote ValidateProductDetails(LifeQuote objLifeQuote)
        {
            using (AVOAIALifeEntities entity = new AVOAIALifeEntities())
            {
                ...
                long MaxHEC = 0;
                List<int> ChildHospInc = new List<int>();
                List<int> ChildHospCareBenifit = new List<int>();
                List<int> ChildHospBenifit = new List<int>();
                List<int> ChildHospExp = new List<int>();
                List<int> FamChildHospExp = new List<int>();
                List<int> ChildIncpExp = new List<int>();
                //decimal APCP = 
                for (int i = 0; i < objLifeQuote.objQuoteMemberDetails.Count; i++)
                {
                            ...
                            else
                            {
                                if (objLifeQuote.objProductDetials.PlanCode == "PPH")
                                {
                                    ...
                                    if (objLifeQuote.objQuoteMemberDetails[i].Assured.StartsWith("Child"))
                                    {
...

                                        {
                                            ...
                                            ChildHospCareBenifit.Add(Convert.ToInt32(objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails[j].RiderSuminsured));
...

                                        {
                                            ...
                                            ChildHospBenifit.Add(Convert.ToInt32(objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails[j].RiderSuminsured));
...

                                        {
                                            ...
                                            ChildHospExp.Add(Convert.ToInt32(objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails[j].RiderSuminsured));

                                        }
                                        if (objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails[j].RiderCode == "TFRA")

                                        {
                                            FamChildHospExp.Add(Convert.ToInt32(objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails[j].RiderSuminsured));
                                ...
                                if (objLifeQuote.objProductDetials.PlanCode == "EPB")
                                {
                                    ...
                                    if (objLifeQuote.objQuoteMemberDetails[i].Assured.StartsWith("Child"))
                                    {
                                        ...
                                        {
                                            ChildIncpExp.Add(Convert.ToInt32(objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails[j].RiderSuminsured));
                                            ChildHospitalizationSA = Convert.ToDecimal(objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails[j].RiderSuminsured);
                                        }
                                        if (objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails[j].RiderCode == "TCRA")
                ...
                if (ChildHospCareBenifit.Any(a => a != ChildHospCareBenifit[0]))
                {
                    objLifeQuote.Error.ErrorMessage += "<br />" + "All children must have same Sum Assured for Child Health Care Cover";

                }
                if (ChildHospBenifit.Any(a => a != ChildHospBenifit[0]))
                {
                    objLifeQuote.Error.ErrorMessage += "<br />" + "All children must have same Sum Assured for Child Hospitalization Cover";

                }
                if (ChildHospExp.Any(a => a != ChildHospExp[0]))
                {
                    objLifeQuote.Error.ErrorMessage += "<br />" + "All children must have same Sum Assured for Child Hospital Expense Cover";

                }
                if (FamChildHospExp.Any(a => a != FamChildHospExp[0]))
                {
                    objLifeQuote.Error.ErrorMessage += "<br />" + "All children must have same Sum Assured for Family Child Hospital Expense Cover";

                }
                if (ChildIncpExp.Any(a => a != ChildIncpExp[0]))
                {
        ...
        public LifeQuote ValidateFHEC(LifeQuote objLifeQuote)
        {
            ...
            List<string> FHECSumAssured = new List<string>();
            for (int i = 0; i < objLifeQuote.objQuoteMemberDetails.Count; i++)
            {
                listRiders.AddRange(objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails.Where(a => a.BenifitOpted == true).Select(a => a.BenefitID).ToList());
                ...
                for (int j = 0; j < objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails.Count; j++)
                {
                    ...
                    if (objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails[j].BenifitOpted)
                    {
                        ...
                        {
                            FHECPPHSelected++;
                            if (!string.IsNullOrEmpty(objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails[j].RiderSuminsured))
                                FHECSumAssured.Add(objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails[j].RiderSuminsured);
                        }
                        if (objLifeQuote.objQuoteMemberDetails[i].ObjBenefitDetails[j].RiderID == 15)
                        {
                            FHECHPASelected++;
            ...
            if (FHECSumAssured.Distinct().Count() > 1)
            {
                objLifeQuote.Error.ErrorMessage += "<br />" + "Family Hospital Expense Cover is Sum Assured should remain same for all members";
            }
            return objLifeQuote;
