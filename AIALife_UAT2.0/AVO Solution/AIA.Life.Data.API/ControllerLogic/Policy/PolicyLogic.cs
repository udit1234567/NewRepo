...
namespace AIA.Life.Data.API.ControllerLogic.Policy
{
                    ...
                    if (ObjPolicy.HdnDocumentDetails != null)
                    {
                        ...
                        if (objLstDoc != null && objLstDoc.Count() > 0)
                        {
                            ...
                            foreach (var Document in objLstDoc)
                            {
                                ...
                                {
                                    DeleteExisitingPolicyDocuments(DocumentpolicyID);
                                }
 ...
 on po.BenifitID equals pa.ProductPlanRiderId
        ...
        public void DeleteExisitingPolicyDocuments(decimal PolicyID)
        {
            try
            {
                AVOAIALifeEntities Context = new AVOAIALifeEntities();
                var objPOlicyDocumnets = Context.tblPolicyDocuments.Where(a => a.PolicyID == PolicyID && a.ItemType == "PolicyDocuments" && a.FilePath == null).ToList();
                if (objPOlicyDocumnets != null)
                {
                    Context.tblPolicyDocuments.RemoveRange(objPOlicyDocumnets);
                    Context.SaveChanges();
                }
            }
            catch (Exception ex)
                    ...
                    if (ObjPolicy.HdnDocumentDetails != null)
                    {
                        ...
                        if (objtblpolicy != null)
                        {
                            ...
                            if (objLstDoc != null && objLstDoc.Count() > 0)
                            {
                                foreach (var Document in objLstDoc)
                                {
                                    tblPolicyDocument objPolicyDocument = objtblpolicy.tblPolicyDocuments.Where(a => a.FileName == Document.FileName && a.MemberType == Document.MemberType && a.DocumentUploadID == Document.DOCID).FirstOrDefault();
                                    if (objPolicyDocument == null)
                                        objPolicyDocument = new tblPolicyDocument();
                                    objPolicyDocument.FileName = Document.FileName;
                                    #region Set Document Type
                            ...
                            if (!ObjPolicy.ProcceedToPayment)
                            {
                                ...
                                var Message = objLogic.ValidateDeviation(ObjPolicy);
                            }
        ...
        public AIA.Life.Models.Common.MemberDetails UpdateDocumentStatus(AIA.Life.Models.Common.MemberDetails objMemberDetails, string UserID, Guid CommonGuid, bool IsIntermSave, decimal PolicyID)
        {
            try
            {
                using (AVOAIALifeEntities Context = new AVOAIALifeEntities())
                {
                    ...
                    {
                        foreach (var MedicalDocument in Context.tblPolicyDocuments.Where(a => a.FileName == MedicalDOc.Document && a.ItemType == "PolicyDocuments" && a.MemberType == objMemberDetails.AssuredName && a.PolicyID == PolicyID && a.DocumentUploadID == MedicalDOc.DocumentId).ToList())
                        {
                    ...
                    {
                        foreach (var NonMedicalDocument in Context.tblPolicyDocuments.Where(a => a.FileName == NonMedicalDOc.Document && a.ItemType == "PolicyDocuments" && a.MemberType == objMemberDetails.AssuredName && a.PolicyID == PolicyID && a.DocumentUploadID == NonMedicalDOc.DocumentId).ToList())
                        {
        ...
        public DeletePolicyDocuments DeletePolicyDocument(DeletePolicyDocuments ObjPolicyDocuments)
        {
            try
            {
                using (AVOAIALifeEntities Context = new AVOAIALifeEntities())
                {
                    var DocList = Context.tblPolicyDocuments.Where(a => a.MemberType == ObjPolicyDocuments.MemberType && a.FileName == ObjPolicyDocuments.DocumentName).ToList();
                    if (DocList != null)
                    {
                        Context.tblPolicyDocuments.RemoveRange(DocList);
                        Context.SaveChanges();
        ...
        public void SendDocumentsEmail(AIA.Life.Models.Policy.Policy ObjPolicy)
        {
            try
            {
                ...
                var CreatedBy = new Guid(objtblPolicy.Createdby);
                var LoginId = Context.tblUserDetails.Where(a => a.UserID == CreatedBy).Select(a => a.LoginID).FirstOrDefault();
                var WPEmail = Context.tblMasIMOUsers.Where(a => a.UserName == ObjPolicy.UserName).Select(a => a.Email).FirstOrDefault();
                var WPMobile = Context.tblMasIMOUsers.Where(a => a.UserName == LoginId).Select(a => a.MobileNo).FirstOrDefault();
